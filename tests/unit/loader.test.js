...
describe('Loader', function () {
  it('should have correct inital values', function () {
    ...
    expect(typeof loader.xhr).toBe('object');
    expect(typeof loader.onAssetLoaded).toBe('object');
  });
  ...
  it('should add an image to the queue', function () {
    ...
    expect(loader.queue[1].name).toBe('brick');
    expect(loader.queue[1].url).toBe('../assets/audio/brick.png');
    expect(loader.queue[1].type).toBe('image');
  });

  it('should add JSON to the queue', function () {
    loader.addJSON('test', '../assets/json/test.json');
    expect(loader.queue.length).toBe(3);
    expect(loader.queue[2].name).toBe('test');
    expect(loader.queue[2].url).toBe('../assets/json/test.json');
    expect(loader.queue[2].type).toBe('json');
  });
});
